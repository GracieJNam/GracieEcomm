...
<?php
    ...
    $password = hash('sha256', $password); //generate the hashed password with the salt value
     ...
     $sql = "SELECT * FROM admin WHERE adminUserName='$username'"; //check if the username is taken
     ...
     elseif ($username == "" || $password == "" || $firstName == "" || $lastName == "" || $mobile =="" || $email == "") //check if all required fields have data
     {
     ...
     {
         $sql="INSERT INTO admin (adminUserName, adminPassword, adminFirstName, adminLastName, adminMobile, adminEmail, adminJoinDate, accountId) VALUES ('$username', '$password',  '$firstName', '$lastName','$mobile', '$email', NOW(), '2')";
         $result = mysqli_query($con, $sql) or die(mysqli_error($con)); //run the query
