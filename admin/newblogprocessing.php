...
<?php
     ...
     if($_FILES['image']['name']) //if an image has been uploaded
     {
         ...
         if($_FILES['image']['size'] > 512000) //image maximum size is 500kb
         {
             ...
             header("location:blognew.php"); //redirect to registration.php
             exit();
         }
     ...
     $sql="INSERT INTO blog (blogTitle, blogContent, datePosted, categoryId, authorId, blogImage) VALUES ('$title', '$content', NOW(),'$categoryId','$adminId', '$newImageName')";
    $result = mysqli_query($con, $sql) or die(mysqli_error($con)); //run the query

     $_SESSION['success'] = 'New blog post successfully added.'; //if new review is added successfully initialise a session called 'success' with a msg
     header("location:blognew.php"); //redirect to index.php
